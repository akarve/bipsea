    ...
    for vector in TEST_VECTORS:
        ...
        for ch, tests in vector["chain"].items():
            for type_, expected in tests.items():
                if type_ == "ext pub":
                    print(f"\nderive {ch} @ {type_}")
                    derived = derive_key(seed, ch, mainnet=True, private=False)
                # assert str(derived) == expected
                elif type_ == "ext prv":
                    print(f"derive {ch} @ {type_}")
                    derived = derive_key(seed, ch, mainnet=True, private=True)
                    # assert str(derived) == expected
                else:
                ...
                if not str(derived) == expected:
                    ...
                    print(derived)
                    print(expected)
                    print("derived", repr(derived))
                    print("expected", repr(parse_ext_key(expected)))
                    print()
