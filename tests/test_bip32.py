from bip32 import gen_master_key
...
def test_vectors():
    for _, vector in TEST_VECTORS.items():
        seed = bytes.fromhex(vector["seed_hex"])
        for chain, tests in vector["chain"].items():
            for type_, expected in tests.items():
                if type_ == "ext pub":
                    assert str(gen_master_key(seed)) == expected
                elif type_ == "ext prv":
                    assert str(gen_master_key(seed, private=True)) == expected
                else:
